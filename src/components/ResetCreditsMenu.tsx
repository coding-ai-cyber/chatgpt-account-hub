...
function formatExpiryDetail(expiresAt: string | null, t: Translate): string {
  ...
  return t("expiresOn", { date: expiry });
}
...
export function ResetCreditsMenu({
  ...
  const countLabel = count === 1 ? t("oneReset") : t("multipleResets", { count });
  ...
  const nextExpiryLabel =
    nextExpiry === "No expiry"
      ...
      : nextExpiry === "Expiry unavailable"
        ? t("expiryUnavailableLower")
        : t("closestExpiry", { date: nextExpiry });
  ...
  return (
    <div ref={wrapperRef} className="relative min-w-0 max-w-full">
      <button
        ...
        title={`${countLabel} · ${nextExpiryLabel} · ${t("clickForExpiryDetails")}`}
      >
        <span
          className={
            compact
      ...
      {isOpen && (
        <div
          id={popupId}
          role="dialog"
          aria-label={t("resetCreditExpiryDetails")}
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="truncate font-medium text-gray-800 dark:text-gray-200">
                    {credit.title?.trim() || t("resetOrdinal", { count: index + 1 })}
                  </div>
