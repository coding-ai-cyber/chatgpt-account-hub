...
function getSubscriptionStatus(timestamp: string | null | undefined, t: Translate, locale: string): {
  label: string;
  className: string;
} {
  if (!timestamp) {
  ...
  if (remainingMs <= 0) {
    return {
      label: t("expiredOn", { date: formattedDate }),
  ...
  if (remainingMs <= 3 * 24 * 60 * 60 * 1000) {
    return {
      label: t("untilDate", { date: formattedDate }),
      className: "text-red-500 dark:text-red-400",
    };
  }
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs mb-3">
        <div className="text-gray-400 dark:text-gray-500">
          {t("lastUpdated", { date: formatLastRefresh(lastRefresh, t, locale) })}
        </div>
      ...
      <div data-account-action className="flex gap-2 mt-3">
        ...
        {account.is_active ? (
          ...
          <button
            ...
            title={codexRunning ? t("closeRunningCodexProcessesAndSwitchAccount") : undefined}
          >
        ...
        <button
          ...
          title={warmingUp ? t("sendingWarmupRequest") : t("sendMinimalWarmupRequest")}
        >
          ⚡
        </button>
        {onToggleAutoWarmup && (
          <button
            ...
            title={
              autoWarmupManagedByAll
                ? t("autoWarmupEnabledForAllAccounts")
                : autoWarmupEnabled
                  ? t("disableAutoWarmupForThisAccount")
                : t("enableAutoWarmupForThisAccount")
            }
          >
            <span className="flex items-center gap-1">
              <span>♻</span>
              <span>{autoWarmupLabel ?? (autoWarmupEnabled ? t("enabled") : t("disabled"))}</span>
            </span>
          </button>
        )}
        <button
          ...
          title={statsOpen ? t("hideUsageStatistics") : t("showUsageStatistics")}
        >
          <svg
            className="h-4 w-4"
            viewBox="0 0 24 24"
