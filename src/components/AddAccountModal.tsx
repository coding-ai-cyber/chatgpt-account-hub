...
export function AddAccountModal({
  ...
  const handleSelectFile = async () => {
    try {
      const selected = await pickAuthJsonFile(t);
  ...
  const handleImportFile = async () => {
    if (!fileSource) {
      setError(t("pleaseSelectAuthFile"));
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl w-full max-w-md mx-4 shadow-xl">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-800">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t("addAccount")}</h2>
          <button
            onClick={handleClose}
            aria-label={t("close")}
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {t("accountNameOptional")}
            </label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder={t("leaveBlankToUseEmail")}
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              {oauthPending ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-gray-700 dark:text-gray-300 font-medium mb-2">{t("waitingForBrowserLogin")}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
                    {t("openFollowingLoginLink")}
                  </p>
                    ...
                    <button
                      onClick={() => {
                        void navigator.clipboard
                          ...
                          .catch(() => {
                            setError(t("clipboardUnavailableCopyLinkManually"));
                          });
                      }}
            ...
            <div>
              ...
              <div className="flex gap-2">
                <div className="flex-1 px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-600 dark:text-gray-300 truncate">
                  {describeFileSource(fileSource, t)}
                </div>
                <button
