...
use crate::commands::{
    ...
    get_language, get_masked_account_ids, import_accounts_full_encrypted_bytes,
...
async fn invoke_web_command(command: &str, payload: Value) -> Result<Value, String> {
    match command {
        ...
        "get_language" => to_json(get_language()),
        "set_language" => {
            let args: LanguageArgs = parse_args(payload)?;
            crate::commands::window::persist_language(&args.language)?;
            Ok(json!({}))
        }
